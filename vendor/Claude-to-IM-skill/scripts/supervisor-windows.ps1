...
function Test-PidAlive {
    param([string]$BridgePidParam)
    if (-not $BridgePidParam) { return $false }
    try { $null = Get-Process -Id ([int]$BridgePidParam) -ErrorAction Stop; return $true }
    catch { return $false }
}
...
function Show-FailureHelp {
    ...
    Write-Host "  3. Rebuild bundle:   cd `"$SkillDir`"; npm.cmd run build"
}
    ...
    'stop' {
        ...
        if ($svc -and $svc.Status -eq 'Running') {
            ...
            if (Test-Path $StatusFile) {
                ...
                $status | ConvertTo-Json -Depth 6 | Set-Content $StatusFile
            }
        } else {
            $bridgePid = Read-Pid
